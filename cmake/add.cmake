...
function(hook_target_include_dirs_impl
	target
	system
	includes
)
...
function(hook_target_include_dirs
	target
)
	absolute_include_dirs(_ALL_INCS ${ARGN})
	hook_target_include_dirs_impl(${target} FALSE "${_ALL_INCS}")
endfunction()

function(hook_target_include_dirs_sys
	target
)
	absolute_include_dirs(_ALL_INCS ${ARGN})
	hook_target_include_dirs_impl(${target} TRUE "${_ALL_INCS}")
endfunction()
...
function(hook_user_header_search_paths
	name
	includes
)
	if(XCODE)
...
function(hook_target_link_libraries
	target
	library_deps
)
...
function(hook_add_lib__impl
	...
	hook_target_include_dirs(${name} ${includes})
	hook_target_include_dirs_sys(${name} ${includes_sys})

	if(library_deps)
		hook_target_link_libraries(${name} "${library_deps}")
	endif()

	# works fine without having the includes
	# listed is helpful for IDE's (QtCreator/MSVC)
	hook_source_group("${name}" "${sources}")
	hook_user_header_search_paths("${name}" "${includes}")
...
function(hook_add_lib
	...
	hook_add_lib__impl(${name} "${sources}" "${includes}" "${includes_sys}" "${library_deps}")

	set_property(GLOBAL APPEND PROPERTY hook_LINK_LIBS ${name})
endfunction()

# only MSVC uses SOURCE_GROUP
function(hook_add_dll__impl
	name
	sources
	includes
	includes_sys
...
function(hook_add_dll
	...
	hook_add_dll__impl(${name} "${sources}" "${includes}" "${includes_sys}" "${library_deps}")
...
function(hook_add_test_executable name sources includes includes_sys library_deps)
	add_cc_flags_custom_test(${name} PARENT_SCOPE)
	...
	hook_target_include_dirs(${target_name} includes)
	hook_target_include_dirs_sys(${target_name} includes_sys)
	
	target_link_libraries(${target_name} hook::intern::testing)
