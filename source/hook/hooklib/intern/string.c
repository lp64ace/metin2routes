...
char *LIB_strndupN(const char *text, const size_t length) {
	...
	HOOK_assert_msg(LIB_strnlen(text, length) == length, "LIB_strlen(text) must be greater or equal to 'length'!");
...
char *LIB_strncpy(char *dst, size_t dst_maxncpy, const char *src, size_t length) {
	size_t len = LIB_strnlen(src, HOOK_MIN(length, dst_maxncpy - 1));
