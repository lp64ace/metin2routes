...
HOOK_STATIC bool LIB_path_is_absolute_win32(const char *path) {
...
HOOK_STATIC bool LIB_path_combine(char *dst, size_t maxncpy, const char *left, const char *right) {
	HOOK_assert(maxncpy != 0);
	HOOK_assert(dst != left && dst != right);
}
