...
HOOK_INLINE bool LIB_listbase_is_empty(const ListBase *listbase) {
	return (listbase->first == NULL && listbase->first == listbase->last);
}

HOOK_INLINE bool LIB_listbase_is_single(const ListBase *listbase) {
	return (listbase->first && listbase->first == listbase->last);
}

HOOK_INLINE void LIB_listbase_clear(ListBase *listbase) {
	listbase->first = listbase->last = NULL;
}

HOOK_INLINE void LIB_listbase_swap(ListBase *lb1, ListBase *lb2) {
