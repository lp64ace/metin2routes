...
#ifdef WIN32
int LIB_stat(const char *filepath, HookFileStat *r_stat) {
#	if defined(_MSC_VER)
...
bool LIB_exist(const char *filepath) {
	HookFileStat info;
	return (LIB_stat(filepath, &info) == 0) && (info.st_mode & S_IFMT) == S_IFREG;
}

uint64_t LIB_read(int fd, void *buffer, size_t size) {
	uint64_t nbytes_read_total = 0;
	while (true) {
		int nbytes = read(fd, buffer, HOOK_MIN(size, INT_MAX));
		if (nbytes == size) {
...
HOOK_STATIC size_t lib_filelist_path(char out[MAX_PATH], const char *dirname) {
	if (!LIB_path_absolute(out, MAX_PATH, dirname)) {
		return 0;
	}
	size_t length = LIB_strnlen(out, MAX_PATH);
	HOOK_assert(length + 3 < MAX_PATH);
...
HOOK_STATIC int lib_filelist_type(WIN32_FIND_DATA *e) {
...
HOOK_STATIC size_t lib_filelist_dir_contents_win32(const char *dirname, DirEntry **r_list) {
...
#else
HOOK_STATIC int lib_filelist_type(struct dirent *e) {
#	define MAP(unix, HOOK) \
		case unix:          \
			return HOOK;
	switch (e->d_type) {
		MAP(DT_REG, RDT_REG);
...
HOOK_STATIC size_t lib_filelist_dir_contents_unix(const char *dirname, DirEntry **r_list) {
